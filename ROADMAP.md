...
> 状态：**W1–W4 + CLI 已随 commit `57b963a` 发布**。
> **P2 战略抉择已于 2026-08-24 定调**——候选池交给 sleek 的 todo.txt，档位从 10 降到 7，
> 详见下方「sleek 互通改造」与 `docs/SLEEK-INTEROP.md`。
> 逐项保留「验收标准」与「待定子决策」以备回溯。
...
## ✅ P2 · 战略抉择 — 已定调（2026-08-24，见「sleek 互通改造」）

原来的问题是：**简化优先级档位**（当时 A-F/S/N/G/H 共 10 档，录入税高）
**vs 上同步/移动**（单机 + 本地存储锁死扩展）。

答案是**两条一起，而且靠同一个决定**：候选池交给 sleek 的 `todo.txt`。

- 档位从 10 降到 7（A–E 加完成/取消两种状态）——因为 F/N/S 表达的都是「还没进入本单元」，
  那是共享文件里一行的状态，不是这里的一个优先级。
- 「同步」不再需要自研：todo.txt 是纯文本，用户已有的任何同步方案（iCloud / Dropbox / git）
  都直接生效，移动端也已经有一堆 todo.txt 客户端。
- 议会那个问题（只留 3 个机制留哪 3 个）实际答案：**配额 / 2 天单元 / 复盘**。
  这三个是 sleek 没有、而这个项目提供的东西；其余是锦上添花。

---

## ✅ sleek 互通改造（2026-08-24）

**目标**：候选池交给 [sleek](https://github.com/ransome1/sleek) 的 `todo.txt`，
本项目只做「未来两天真正要做的事」这一层。设计正本：`docs/SLEEK-INTEROP.md`。

分七期实施，每期结束仓库都是绿的并单独提交：

| 期 | commit | 内容 |
|----|--------|------|
| P3 | `9a55d9b` | `Recurrence` 结构对齐 todo.txt 的 `rec:[+]<n><d\|b\|w\|m\|y>` 语法 |
| P2 | `d39c0cf` | 单元周期改为 周一+周二 / 周三+周四 / 周五+周六，周日复盘 |
| P1 | `e000497` | `todotxt.ts`：与 sleek 所用解析器逐字节兼容的读写层 |
| P4 | `4d07f14` | 删除 F/N/S 档位与内嵌子任务；配额溢出改为退回候选池 |
| P5+P6 | `3449ad8` | 拉取面板、写回、文件路径设置、主动/被动标记 |
| P7 | — | 文档 / CLI / i18n 同步 |

**四个关键决策**（每个都在规格里有对应章节）：

1. **sleek 当收件箱，单向拉取**。拉取是复制，源行不动；写回只有两个单 token 的动作
   （完成时的 `x`、拉取时的 `@主`/`@被`）。定位不到源行就**不写**——写错一行等于改掉
   别人的任务，代价远大于「没写回」。
2. **F / N 直接删**。「还没分类」= 文件里还没被拉取的行；「重要但不急」= todo.txt 原生的
   `t:` 阈值日期或 `h:1`。
3. **S 变成 `+project`**。本周焦点项目是一个设置项（`focusProject`），子任务变成带同一个
   标签的普通任务——因此每条都能有自己的优先级、截止日和番茄估计，这是原来的勾选清单给不了的。
4. **主动/被动用 `@主` / `@被` context**。存成 context 而不是 Task 字段，好处是只有一份真相：
   自动随文件往返，sleek 侧边栏免费给筛选和计数。未标注的单独计数，不并入任何一侧。

**顺带修掉的真 bug**（都是这次对齐暴露出来的）：

- `+project` / `@context` / `#tag` 原来在词中间也匹配，且会把匹配到的串从正文里删掉：
  `mail bob@example.com about c++ and issue#42` 会变成 `mail bob about c and issue`。
  现在统一用 todo.txt 的空白边界规则。
- `completeTask` 只看 `recurrence.pattern`，所以 `rec:mon,wed,fri` 的任务永远不会重新生成。
- `ReviewPanel` 用 `toISOString()` 取单元日期，UTC 以东会把复盘记到前一天。
- `escapeHtml` 依赖 `document`，让整个 `parser.ts` 实际上不是 Node-safe 的（CLI 也 import 它）。

**数据迁移 4.0 → 5.0**：删档位不等于删数据。F/N 任务、S 的子任务、以及优先级字母已经不认识的
任务，全部进 `pendingExport` 并在主界面挂横幅，直到导出成功才从 `active.json` 里移除。
存量 recurrence 统一打 `strict: true`，保证没有任何现有循环任务因为这次改造而改变到期日。

**验收**：203 单元测试双时区通过；typecheck / svelte-check / build / cargo check 全绿；
浏览器实测覆盖拉取、`@主` 写回、完成写回（`(A)` 按 sleek 规则挪进 `pri:A`）、
源行被改写时的拒绝写回、以及带 F/N/S 的 4.0 数据迁移（4 进 6 出，一条不丢）。

**唯一新增依赖**：`tauri-plugin-dialog`（只开 `dialog:allow-open`），用于选择 todo.txt。
文件读写走自定义 Rust 命令而不是 fs 插件——后者按构建期作用域校验路径，用它就得放开
`fs:allow-*-recursive`，等于把整个文件系统交给 webview。
